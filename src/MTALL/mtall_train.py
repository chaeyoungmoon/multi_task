...
#     print('Available sdevices: ', torch.cuda.device_count())
    ...
    # train and test the model for all targets

    # set seed
    set_seed(args.seed)

    # get the cluster
    # gather all targets in cluster order
    num_clusters = len(cluster_tasks)
    all_tasks = np.array([], dtype=int)
    for k in range(num_clusters):
        cur_tinds = cluster_tasks[k]['tinds']
        all_tasks = np.append(all_tasks, cur_tinds)
    print_and_log(f"total targets: {len(all_tasks)} ({all_tasks[:15]})")
    ...
    logfname = os.path.join(logdir, f'MT.all.train.log')
    logging.basicConfig(filename=logfname,
                        format='%(asctime)s %(levelname)-8s %(message)s',
                        level=logging.INFO, datefmt='%Y-%m-%d %H:%M:%S', 
                        force=True)
    ...
    print_and_log(f'hidden1: {args.hidden1} | hidden2: {args.hidden2} | drop-rate: {args.drop_rate}')
    print_and_log(f'result dir: {resdir}')
    print_and_log(f'current cluster: all | num tasks: {num_tasks}')
